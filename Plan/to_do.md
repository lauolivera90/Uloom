...
[x] Selector de paleta de colores: ampliar el sistema de temas más allá de claro/oscuro, permitiendo elegir entre paletas predefinidas (o personalizar colores). Requiere revisar el sistema de tokens actual antes de sumar paletas nuevas. → Resuelto en v0.5.2: 5 paletas predefinidas (violeta default, azul, esmeralda, ámbar, rosa) que redefinen SOLO los tokens de identidad (`primary`/`primary-hover`/`accent`/`on-primary`) en claro y oscuro vía el atributo `data-palette` en `index.css`; catálogo de ids en `shared/lib/palettes.js` (sin colores — CSS es la fuente), anti-flash con allowlist en `public/theme-init.js`, persistencia en `localStorage['uloom-palette']` y runtime en `useTheme` (`{ palette, setPalette }`). Selector "Paleta de colores" en Configuración → Preferencias con el widget `PalettePicker` (swatches en vivo por `data-palette` en el subárbol, radio group). Contraste AA verificado (≥4.5:1 en `on/fill`, `hover/bg`, `accent/bg`). La personalización manual de colores (picker) queda fuera de esta versión.

v0.5.3 Gestión de sesiones avanzada
[x] Duplicar sesión: clonar una sesión existente (tabs, configuración de lanzamiento) generando un nuevo id, con nombre sugerido tipo "Copia de [nombre]". → Resuelto en v0.5.3 con flujo de modal precargado (nombre/icono/descripción editables antes de confirmar), clonado en el backend vía `workspace:duplicate` (tabs con ids nuevos, fuente intacta) y nombre sugerido con semántica `root (count)`: `hola` → `hola (1)` → `hola (2)` (nunca apila sufijos; el roadmap decía "Copia de [nombre]", se reemplazó por la semántica `(N)`). Botón en el Detalle (header) y toast de éxito (`duplicate.success` — resultado no visible desde el Detalle).

v0.5.4 Historial de pestañas
[x] Historial/reutilización de tabs al agregar: sección en el modal de agregar pestaña con las últimas tabs agregadas o las más usadas entre sesiones, para reutilizarlas sin reescribir la URL. Requiere un registro liviano de tabs usadas (URL + nombre + frecuencia/última vez), separado de las sesiones. → Resuelto en v0.5.4: registro en `config.tabHistory` (cap 20, orden `count` desc → `lastUsedAt` desc, upsert por URL) alimentado desde el backend al **agregar** (diff de `workspace:update`, individual o en lote) y al **lanzar** (best-effort). El modal de agregar pestaña pasó a **dos modos** (segment control Manual | Historial, patrón de Configuración): el modo Historial es **selección múltiple** con checkboxes que excluye las URLs ya presentes y confirma el lote en una sola escritura (`addTabs`). El segment se deshabilita cuando no hay nada para mostrar (decidido antes de abrir: la lista se carga al montar y refresca en cada apertura). Limpieza en Configuración → Sesiones con toast. No se exporta/importa (caché local de uso).
