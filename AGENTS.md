...
3 páginas: Hub de Sesiones, Detalle de Sesión, Configuración. Navegación principal con Sidebar colapsable (modelo push, persistencia en localStorage, item activo por ruta). La página de Configuración sigue siendo mayormente maqueta: tabs Preferencias/Sesiones, toggle de Tema ilustrativo y placeholders de export/import (la portabilidad real es v0.4.x), salvo la fila "Navegador predeterminado", que es funcional y hoy muestra el ícono del navegador efectivo. Sistema de tokens con la regla "fill vs foreground" (`primary` = fill, `primary-hover` = foreground) y `accent` para identidad no interactiva (ver `.doc/design.md`). El **Motor de Lanzamiento sigue operativo** y las consultas cacheadas de navegadores comparten el hook genérico `useCachedQuery`. La hoja de ruta por versión vive en `Plan/to_do.md`.
...
Este proyecto está planeado para escalar en fases futuras. Esta sección se actualiza en cada fase nueva — no asumas que el scope de v0.3.2 es el scope final del proyecto, pero tampoco adelantes funcionalidad de fases futuras sin que se documente acá primero.
