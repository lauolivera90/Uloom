...
**Nuevo en v0.2.2 — Configuración de Navegador por Sesión:** la card Configuración del Lienzo dejó de ser placeholder. Define `openBehavior` (Ventana activa/nueva, default `active-tab`) y `browser` por sesión, con herencia de 3 niveles para el lanzamiento (v0.3): **sesión → global → sistema** (`sesión.browser ?? (global !== 'system' ? global : null)`). Solo se ofrecen navegadores **instalados** detectados por probe de rutas (`browser:list` → `browserService`, solo win32). `preferences.defaultBrowser` es una preferencia global funcional vía `config:updatePreferences` (merge parcial); escritura pesimista inmediata. Backend separado por dominio para browser/preferences; el refactor de los archivos monolíticos restantes (`configRepository`, `configService`, `ipc/index.js`) es v0.2.3. Fuera de esta fase: Launch real (v0.3), refactors pendientes del reviewer (v0.2.3), portabilidad/temas (v0.4).

Este proyecto está planeado para escalar en fases futuras. Esta sección se actualiza en cada fase nueva — no asumas que el scope de v0.2.2 es el scope final del proyecto, pero tampoco adelantes funcionalidad de fases futuras sin que se documente acá primero.
