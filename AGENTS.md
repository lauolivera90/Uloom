...
3 páginas: Hub de Sesiones, Detalle de Sesión, Configuración. Navegación principal con Sidebar colapsable (modelo push, persistencia en localStorage, item activo por ruta). La página de Configuración sigue siendo mayormente maqueta: tabs Preferencias/Sesiones, toggle de Tema ilustrativo y placeholders de export/import (la portabilidad real es v0.4.x), salvo la fila "Navegador predeterminado", que es funcional. Sistema de tokens con la regla "fill vs foreground" (`primary` = fill, `primary-hover` = foreground) y `accent` para identidad no interactiva (ver `.doc/design.md`). El **Motor de Lanzamiento está operativo**: la sesión se abre en el navegador resuelto por 3 niveles (sesión → global → sistema) por spawn del ejecutable con bandera de ventana nueva según `openBehavior`, quedando `shell.openExternal` solo como fallback; botón `Launch` en el Detalle y play de sesión en cada card del Hub. La hoja de ruta por versión vive en `Plan/to_do.md`.

**Nuevo en v0.3.1 — Motor de Lanzamiento (Disparador Principal):** el botón `Launch` (Detalle, variante primary, reemplaza el placeholder "Disponible en v0.3") y el play de sesión de `WorkspaceCard` (Hub) quedan cableados al canal IPC `workspace:launch`. En el main, `launcherService.launchWorkspace(id)` resuelve el navegador efectivo (`sesión.browser ?? (preferences.defaultBrowser !== 'system' ? preferences.defaultBrowser : null)`) y abre cada `tab.url`: navegador concreto → `spawn` del ejecutable con `{ detached, stdio: 'ignore' }` + `unref()` y bandera de ventana nueva por motor (`--new-window` en Chromium — chrome/edge/brave/opera/vivaldi — y `-new-window` en firefox) si `openBehavior === 'new-window'`, o solo la URL en `active-tab`; navegador de sistema → `app.getApplicationInfoForProtocol('https:')` resuelve su ejecutable y se spawna igual (bandera por heurística de motor del ejecutable); si no se puede resolver, cae a `shell.openExternal` (único caso sin control de ventana nueva — bugfix verificado: con "Predeterminado (Sistema)" + ventana nueva el default se spawna y abre en ventana nueva). Devuelve `{ opened, failed }`: los fallos por URL se cuentan, los estructurales (sesión o navegador configurado no instalado) lanzan `{ success: false }`. En renderer: `entities/workspace/hook/useLaunchWorkspace` (compartido Hub/Detalle, nunca lanza) y la constante `LAUNCH_EMPTY_TABS_TITLE`; los botones van `disabled` mientras se lanza o si la sesión no tiene pestañas. Deuda de accesibilidad de `WorkspaceCard` cerrada (el play es una acción real). Pase del @reviewer: `useLaunchWorkspace` y `useInstalledBrowsers` migrados de `shared/hook/` a `entities/workspace/hook/` (rompe el ciclo `shared ↔ entities` detectado por lint) e `import/no-cycle` habilitado en `.eslintrc.json`. Fuera de esta fase: portabilidad/temas (v0.4).
...
Este proyecto está planeado para escalar en fases futuras. Esta sección se actualiza en cada fase nueva — no asumas que el scope de v0.3.1 es el scope final del proyecto, pero tampoco adelantes funcionalidad de fases futuras sin que se documente acá primero.
