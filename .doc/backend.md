...
- `workspaceService.js`: `getConfig()` (config completa normalizada), `createWorkspace()` (genera id con randomUUID, arma `tabs: []`, `openBehavior: 'active-tab'`, `browser: null` y `pinned: false` — v0.6.1), `duplicateWorkspace(sourceId, input)` (v0.5.3: lee la sesión fuente con `getWorkspaceById`, clona sus `tabs` con **ids nuevos** y copia `openBehavior`/`browser` en un workspace nuevo con id nuevo y los datos básicos provistos — la fuente nunca se modifica; desde v0.6.1 el clon arranca `pinned: false`, no hereda el fijado de la fuente), `updateWorkspace()` (**v0.5.4:** lee la sesión previa con `getWorkspaceById`, persiste el update estricto y registra en el historial las pestañas con URL nueva por diff — el alta de pestañas no tiene canal propio, va por `workspace:update`; editar/borrar no re-registra; el registro es best-effort y nunca condiciona el resultado del update; el toggle de `pinned` también va por acá y no registra nada porque no cambian las pestañas), `deleteWorkspace()` (delega; baja estricta), `clearMetadataCache()` (delega en el repositorio; devuelve la cantidad de favicons removidos) y `deleteAllWorkspaces()` (delega; devuelve la cantidad de sesiones eliminadas preservando `preferences`).
