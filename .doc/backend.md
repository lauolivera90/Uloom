...
- `uloomApi.launchWorkspace(workspaceId)` → invoca el canal `workspace:launch`. Resuelve con `{ opened, failed, workspace }` (desde v0.6.2 `workspace` es la sesión persistida actualizada con los datos de uso, o `null`).
...
| `workspace:launch` | `workspaceId` | `{ opened, failed, workspace }` (URLs abiertas / fallidas; `workspace` = sesión con `lastLaunchedAt`/`launchCount` actualizados si se abrió algo, o `null`) |
...
- `workspaceService.js`: `getConfig()` (config completa normalizada), `createWorkspace()` (genera id con randomUUID, arma `tabs: []`, `openBehavior: 'active-tab'`, `browser: null`, `pinned: false` — v0.6.1 — y `lastLaunchedAt: null`/`launchCount: 0` — v0.6.2), `duplicateWorkspace(sourceId, input)` (v0.5.3: lee la sesión fuente con `getWorkspaceById`, clona sus `tabs` con **ids nuevos** y copia `openBehavior`/`browser` en un workspace nuevo con id nuevo y los datos básicos provistos — la fuente nunca se modifica; desde v0.6.1 el clon arranca `pinned: false`, no hereda el fijado de la fuente; desde v0.6.2 tampoco hereda los datos de uso: `lastLaunchedAt: null`, `launchCount: 0`), `updateWorkspace()` (**v0.5.4:** lee la sesión previa con `getWorkspaceById`, persiste el update estricto y registra en el historial las pestañas con URL nueva por diff — el alta de pestañas no tiene canal propio, va por `workspace:update`; editar/borrar no re-registra; el registro es best-effort y nunca condiciona el resultado del update; el toggle de `pinned` también va por acá y no registra nada porque no cambian las pestañas), `deleteWorkspace()` (delega; baja estricta), `clearMetadataCache()` (delega en el repositorio; devuelve la cantidad de favicons removidos) y `deleteAllWorkspaces()` (delega; devuelve la cantidad de sesiones eliminadas preservando `preferences`).
...
- `workspaceRepository.js`: dueño de la entidad workspace. `normalizeWorkspace()` (rellena `tabs` como array, `openBehavior: 'active-tab'`, `browser: null`, `pinned: false` desde v0.6.1 y `lastLaunchedAt: null`/`launchCount: 0` desde v0.6.2 — migración de configs viejas), `getConfig()` (config completa con workspaces normalizados), `readWorkspaces()`, `getWorkspaceById()` (lectura estricta, usado por el Launcher), `addWorkspace()`, `updateWorkspace()` (update estricto), `recordLaunch(workspaceId)` (**v0.6.2:** actualiza `lastLaunchedAt` al ahora ISO e incrementa `launchCount` — única mutación de uso de la entidad, disparada por el Launcher best-effort; update estricto; devuelve la sesión persistida), `deleteWorkspace()` (baja estricta), `clearMetadataCache()` (remueve `Tab.favicon` de todas las pestañas y persiste; devuelve la cantidad), `deleteAllWorkspaces()` (vacía `workspaces` preservando `preferences`; devuelve la cantidad) e `importWorkspaces(workspaces, { replace })` (v0.4.2: normaliza la lista; con `replace: true` deja las sesiones locales fuera — restauración de respaldo, preserva `preferences`; con `replace: false` agrega las importadas regenarando el id de las que colisionen con una sesión local; persiste y devuelve la lista final).
...
### Lanzar sesión (`workspace:launch`, v0.3.1, + datos de uso en v0.6.2)
```
...
→ navegador de sistema: browserService.resolveSystemBrowser → su ejecutable
   ...
   → si opened > 0: recordTabsBestEffort (historial) + recordLaunch (workspaceRepository,
     best-effort: lastLaunchedAt = ahora, launchCount + 1) → workspace actualizado
→ { opened, failed, workspace } → useLaunchWorkspace invoca onLaunched(workspace)
      (inyectado) → syncWorkspace del estado global se encola en el write-chain y **mergea
      solo lastLaunchedAt/launchCount** sobre la versión actual en estado (sin re-IPC, el main
      ya persistió) — si hubo una mutación concurrente del mismo workspace (ej. toggle de pin),
      el merge no la pisa → el Hub y el Detalle reflejan el nuevo "última vez lanzada"
```
El `openBehavior` se respeta tanto con el navegador explícito como con el predeterminado del sistema resuelto a ejecutable (bandera del motor): `new-window` abre el conjunto completo en una sola ventana nueva y `active-tab` como pestañas en la ventana vigente; solo si el predeterminado no puede resolverse se cae a `shell.openExternal` por URL y el SO decide. En el spawn el resultado es todo o nada (éxito → `opened = N`; fallo → throw → `{ success: false }`); `failed > 0` solo puede darlo el fallback de `openExternal` (fallos parciales por URL). Los errores estructurales (sesión o navegador inexistentes) se convierten en `{ success: false }` (y no registran uso). Desde v0.4.4 el renderer emite feedback visual: error → toast de error (`launch.error`) y `failed > 0` → toast de advertencia con la cantidad (`launch.partialFailure`).
